...
namespace FunctionForApp.DataAndStorage
{
	/// <summary>
	...
	/// </summary>
	public class YamlData<T>
	{
		private string objectDataFilePath = string.Empty;
		private List<T> objs;
		private List<object> objsG = null!;
		private ISerializer serializer;
		private IDeserializer deserializer;
		private YamlStream yamlStream;
		public YamlData(string objectDataFilePath)
		{
		...
		public void ReadAll(int timeoutMilliseconds = -1)
		{
			if (File.Exists(this.objectDataFilePath))
			{
				var task = Task.Run(() =>
				{
					using (var reader = new StreamReader(this.objectDataFilePath)) { this.yamlStream.Load(reader); }
				});
		...
		public bool Add(List<T> newObjs, string keyTypeName)
		{
			...
			if (typeNameProperty != null)
			{
				...
				using (var writer = new StreamWriter(this.objectDataFilePath)) { this.yamlStream.Save(writer); }
